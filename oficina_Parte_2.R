...
# titulo: "Introdução a Estatística Básica e R(RStudio) -2a. Parte"
# autor: "Walter Humberto Subiza Pina"
...
# 
# Podemos usar a função genérica "read.table", ver ?read.table, que apresenta além dos parâmetros 
# padrão, funções de leitura derivadas, como *read.csv2* e *read.delim2*, adequados a nosso 
...
#   | 6 | 1 | -18.15309 |
#   
#   Vamos ver como fica a sua leitura com "read.table":
...
# A função "read.table", tem como padrão de importação o separador de colunas como "," e o separador 
...
# função "read.cvs", especificando as particularidades e com a versão adaptada "read.csv2". 
...
# Forçar variável "Bloco" como fator, ver novamente estrutura e resumo, tem diferenças??
...
#   
#   "Com a os dados de mtcars, me faça uma impressão das estatísticas da variável mpg, e 
# adiciona dois gráficos, um da variável mpg com disp e um outro com mpg e wt". 
...
# só as principais estatísticas de nossos dados. A função é "describe"
...
# 
# Basta selecionar o código e ir no caminho do menú principal e teremos um resultado como esse:
...
# 
# A função "median" do R faz o cálculo da mediana em qualquer vetor. Vejamos a função do 
...
# liberdade quando o cálculo da média usada na fórmula.
# 
# O R tem a função "var", que calcula diretamente a variância
...
#   
#   $$\sigma = \sqrt \sigma^2 = \sqrt{\frac{\sum\limits_{i=1}^{n} \left(x_{i} - \bar{x}\right)^{2}} {n-1}}$$
#   
#   O R tem a função "sd", que calcula diretamente o desvio padrão.
...
# comparando... 
desv.pad(DTM.H)  # função própria

sd(DTM.H)       # função R

sqrt(variancia(DTM.H))

sqrt(var(DTM.H)) #desvio padrão através da variancia
...
save(list = ls(), file = "Oficina_Parte2_all.Rdata")
