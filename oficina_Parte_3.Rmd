...
---
 ...
 O objetivo é mostrar como usar a função _plot_ do pacote _base_
 para fazer gráficos simples de variáveis, histogramas e boxplot
...
---
 ...
 Tenho um arquivo com 300 observacoes cuja media esperada é de 2.5, um desvio padrão perto de 1 e uma distribucao esperada  tipo normal. O que podemos analisar?
...
O anexo **Anscombe** traz um interessante exemplo da utilidade dos gráficos.
...
Os dados podem ser empilhados, usando uma tabela de dupla entrada, nesse caso, vamos classificar por Bloco e Sub Blocos


```{r} 

(conto2 <- table( Sub.Bloco,Bloco))
color <- c("darkslategray1", "darkslategray3", "darkslategray4", "darkorange1")
...
Primeiro uma plotagem simples com alguns controles. Simulamos dados, 300 valores consecutivos e aleatorios entre -5 e +5...veja _?runif_
...
Uma funcao muito útil para ter mais controle dos eixos é  _axis_. Para poder fazer uso da função, primeiro tem de desabilitar o eixo na função _plot()_, incluíndo o parâmetro xaxt="n" e ou yaxt="n".

Com _axis_ podemos mudamos os valores das marcas, tipo de linha, tamanho, espacamento das linhas, cor, etc. 
...
# Ou se desejar pode tirar toda a borda da plotagem e mostrar apenas os eixos.
# note que o R numera os eixos de 1 a 4 em sentido destrógiro a partir da base
...
Geramos uma distribuicação normal assimétrica à direita:
...
O boxplot faz a plotagem dos 5 números principais, ou seja, **mínimo, 1o. quartil, mediana, 3o. quartil e máximo**. As cercas estão localizadas por padrão a 1,5 vezes o IQ, mas isso pode ser mudado.

Boxplot é uma ferramenta importante para a identificação de outliers , veja ?boxplot.
...
Vejamos agora como agrupar por fatores, usamos o arquivo **selecao**..
 ...
 _boxplot(formula, data = ...),      formula x ~ grupos_
...
O pacote RColorBrewer cria cores ou níveis de cinza através de gray(). 
...
```{r}
sessionInfo()
save(list = ls(), file = "Oficina_Parte3_all.Rdata")
```
